...
\begin{itemize}
    ...
    Moreover, the Admin has the responsibility of managing student's report and, eventually, has the power to ban them. Every operation is performed on a separate control panel which the Admn can log into with his personal verified email.
...
\begin{itemize}
	...
	\item \textbf{Domain Model}: Defines the system's core entities. They also include standardized server response, to provide a uniform way to communicate with the client.
	\item \textbf{DAO (Data Access Object)}: Abstracts the persistence layer from the business logic. This package includes an interface layer to provide an easy swap between DAOs implementations.
