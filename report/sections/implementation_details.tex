...
%The Object-Relation Mapper (ORM) package serves as the data persistence layer for the application. Its primary responsibility is to connect the object-oriented domain model used within the business logic layer with the underlying data storage mechanisms.
...
The \texttt{DAOFactory} interface acts as the central factory, defining methods to instantiate specific resource DAOs, such as \texttt{AdminDAO}, \texttt{UserDAO}, and \texttt{ProfDAO}. These resource-specific interfaces define the standard contract for all CRUD (Create, Read, Update, Delete) operations.

As defined in the \texttt{MainApplication} configuration, Dependency Injection is utilized during startup to bind the application controllers to the concrete implementations (\texttt{InMemDAOFactory} or \texttt{PostgreDAOFactory}) based on the environment configuration. This architectural choice guarantees that the business logic remains entirely agnostic of the underlying database technology, significantly enhancing the maintainability and testability of the system.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.8, transform shape]
		...
		\umldep[geometry=-|]{PostgreAdminDAO}{AdminDAO}
