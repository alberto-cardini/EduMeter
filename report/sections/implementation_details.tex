...
\subsubsection{Resource endpointa} Resource endpoints are responsible for HTTP request dispatching and resolution. This encompasses multiple tasks, including authentication management, exception handling, and database interaction for resource management. This layer represents the core of the backend, it glues the system components together and serves as the primary interface for client interaction.

\paragraph{\textbf{Single Responsibility Principle.}} The endpoints architecture has been designed to respect the single responsibility principle. Consequently, each endpoint is dedicated to managing only its specific resource, providing the necessary paths for user communication. Listing \ref{userEndpointCode} demonstrates this approach through the \texttt{UserEndpoint} class. As a representative example, each endpoint is injected with only its corresponding Resource DAO, which serves as its sole point of data access. It is also clearly visible in Figure \ref{UML-endpoints}, in each class attributes and in the fact that classes do not interact with each other. \\

In a couple of cases this principle is crossed. This occurs in those endpoints that, due to their nature, are multiple resources aggregators, for instance:
...
In the \textit{EduMeter} implementation, it was mandatory to implement specific handlers for JSON exceptions, as there are already some implementations of those in the Jackson library, that would come up as more specific during the lookup, and our global handler wouldn't fire.
...
The Object-Relation Mapper (ORM) package has the responsibility of \textbf{connecting the object-oriented domain model} used within the business logic layer \textbf{to the underlying data storage mechanisms}. 
