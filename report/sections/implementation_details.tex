...
%In EduMeter, users and admins needs to authenticate in order to perform their specific actions (publication and management). 

%One of EduMeter's core features is the complete anonymity of its users. This means that both users and admins cannot gather personal information about another user. To achieve this goal the system cipher the student's sensitive data with a robust encoding algorithm called \texttt{HmacSHA256}. Inside the application, users are identified through their personal hash-code, calculated composing their personal institutional email (asked during the login phase), a value called \texttt{Secret} and a

%The \texttt{auth} package implements everything needed by the authentication service procedures. The package is composed by 2 sub-packages, \texttt{annotations} and \texttt{filters}, and 2 stand-alone classes, \texttt{CryptoServices} and \texttt{Token}. 
...
\begin{figure}
	...
	\begin{tikzpicture}[scale=0.6, transform shape]
		\begin{umlpackage}{controllers}
			...
			\umlclass[x=3,y=-15]{PublishedReviewController}{
				+ publishedReviewDAO : 
			}{
...
\begin{figure}
	...
	\begin{tikzpicture}[scale=0.7, transform shape]
		...
		% --- LIVELLO 1: INTERFACCE (In alto, disposte a quadrato/griglia per risparmiare spazio) ---
		...
		% --- LIVELLO 2: PACKAGE CON GLI HANDLER (Sotto i rispettivi Mapper) ---
		\begin{umlpackage}[x=0.5, y=-8]{exceptionHandlers}
			
			% Gli handler sono posizionati esattamente sotto i loro mapper
			...
			% --- RELAZIONI (Linee dritte e pulite) ---
