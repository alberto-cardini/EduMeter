...
\begin{tabularx}{\textwidth}{lX}
	\toprule
	\textbf{UC \#5} & \textbf{Report Review} \\ \midrule
	Level & User Goal\\
	Description &  \\
	Actors & Student \\
	Pre-conditions & The student must be authenticated. \\
	...
	\end{tabenum} \\
	Post-conditions & The report is visible to the admin for validation. \\
...
\begin{tabularx}{\textwidth}{lX}
	\toprule
	\textbf{UC \#6} & \textbf{Admin Login} \\ \midrule
	Level & Function \\
	Description &  The admin log in the system to access his functionality\\
	Actors & Admin \\
	Pre-conditions & The admin email must be stored in the database. \\
	Steps & 
	\begin{tabenum}
	...
	\end{tabenum}\\
	Post-conditions & The admin is logged in and has access to his functionality. \\
	Alternative Steps & 
	\begin{tabitem}
...
\begin{tabularx}{\textwidth}{lX}
	\toprule
	\textbf{UC \#7} & \textbf{Validate Review Information} \\ \midrule
	Level & User Goal \\
...
\begin{tabularx}{\textwidth}{lX}
	\toprule
	\textbf{UC \#8} & \textbf{Validate Report} \\ \midrule
	Level & User Goal \\
	Description & The admin evaluates reported reviews and take moderation actions. \\
	Actors & Admin \\
	Pre-conditions & The admin is logged in. \\
	Steps & 
	\begin{tabenum}
	...
	\end{tabenum} \\
	Post-conditions & The report has been processed and the moderation procedure is consistent in the database. \\
	Alternative Steps & 
	\begin{tabitem}
		\item[3a.] If the review  do not contains any violation, the report is dismissed and the review remains visible.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		[every entity/.style={fill=blue!20,draw=blue,thick},
		 ...
		 % Style for the inheritance arrow
     		 isa arrow/.style={thick,-{Triangle[open, length=8pt, width=8pt]}},
		 % Style for attributes of entities and relations
		...
		% User, reviews, reports, bans
		% \node[relationship](receives){Receives};
		% \node[entity](ban)[below = of receives]{Ban} edge node[right] {0..1} (receives);
		% 	\node[pk, label=above:id](ban_id)[above left = 0.5cm of ban]{} edge (ban);
		% 	\node[attr, label=above:date](date_id)[left = 0.5cm of ban]{} edge (ban);
		...
		\node[entity](admin)[right = 7cm of user]{Admin};
			...
			%\node[attr, label=right:isActive](adminIsActive)[below right = 0.5cm of admin]{} edge (admin);
