...
\begin{usecasetable}{1}{Browse Review}
	\begin{tabularx}{\textwidth}{@{} l|X @{}}
		...
		\begin{tabenum}
			...
			\item The guest scrolls through and reads them (Tests \ref{test:testSearch_Unauth:PubReview}, \ref{test:testGet_Unauthenticated_NotFound:PubReview}).
		\end{tabenum} \\
		...
		\begin{tabitem}
			...
			\begin{itemize}
				...
				\item The system updates the view to show only matching reviews (Test \ref{test:testSearch_Unauth:PubReview}).
			\end{itemize}
		\end{tabitem}
	\end{tabularx}
\end{usecasetable}
...
\begin{usecasetable}{2}{Student Login}
	\begin{tabularx}{\textwidth}{@{} l|X @{}}
		...
		\begin{tabenum}
			...
			\item The system sends a verification code to the provided address (Tests \ref{test:testSendPin_Student_ValidDomain:Auth}, \ref{test:testSendPin_Student_InvalidDomain:Auth}, \ref{test:testSendPin_MissingEmail:Auth}).
			\item The user enters the code (Mock-up \ref{mock-up-4-auth-page-step-2}).
			\item The system verifies if the entered code matches the one sent (Tests \ref{test:testLogin_Vaid:Auth}, \ref{test:testLogin_WrongPin:Auth}, \ref{test:testLogin_ExpiredPin:Auth}, \ref{test:testLogin_MissingFields:Auth}, \ref{test:testLogin_NotFound:Auth}).
			\item The system retrieves the existing unique ID (or generates a new one for first-time access) (Test \ref{test:testLogin_Vaid:Auth}).
		\end{tabenum} \\
...
\begin{usecasetable}{3}{Submit Review}
	\begin{tabularx}{\textwidth}{@{} l|X @{}}
		...
		\begin{tabenum}
			...
			\item The user fills the Teaching field by selecting an existing option from a provided list.
		...
		\textbf{Alternative Steps} & \begin{tabitem}
			...
			\begin{itemize}
				...
				\item Upon submission, the system saves the review but is not published until an Admin approves it (UC \#5: Validate Review Data, Test \ref{test:testPublish:Draft}).
			\end{itemize} 
		\end{tabitem}
	\end{tabularx}
\end{usecasetable}
...
\begin{usecasetable}{6}{Admin Login}
	\begin{tabularx}{\textwidth}{@{} l|X @{}}
		...
		\begin{tabenum}
			...
			\item The system verifies if the email is in the admin whitelist (Test \ref{test:testSendPin_Admin_Valid:Auth}).
			\item The system sends a verification code to the provided address (Tests \ref{test:testSendPin_Admin_Valid:Auth}, \ref{test:testSendPin_MissingEmail:Auth}).
			\item The admin enters the code.
			\item The system verifies if the entered code matches the one sent (Tests \ref{test:testLogin_Vaid:Auth}, \ref{test:testLogin_WrongPin:Auth}, \ref{test:testLogin_ExpiredPin:Auth}, \ref{test:testLogin_MissingFields:Auth}, \ref{test:testLogin_NotFound:Auth}).
			\item The system notifies the successful login (Test \ref{test:testLogin_Vaid:Auth}).
		\end{tabenum} \\
		\textbf{Post-conditions} & The admin is logged in and has access to his functionality. \\
		\textbf{Alternative Steps} & \begin{tabitem}
			\item[3a.] If the provided email is not recognized, the system signal the error and ask the admin to provide a new one (Test \ref{test:testSendPin_MissingEmail:Auth}).
			\item[7a.] If the provided code don't match the one send, the system ask the Admin to re-enter the code or request a new one (Tests \ref{test:testLogin_WrongPin:Auth}, \ref{test:testLogin_ExpiredPin:Auth}).
		\end{tabitem}
	\end{tabularx}
\end{usecasetable}
...
\begin{usecasetable}{7}{Validate Review Information}
	\begin{tabularx}{\textwidth}{@{} l|X @{}}
		...
		\begin{tabenum}
			...
			\item The system creates the new Teaching entry in the database (Test \ref{test:testPublishDraftReview_Success:Draft}).
		\end{tabenum} \\
		\textbf{Post-conditions} & The review is published and the new Teaching entry is available for future reviews. \\
		\textbf{Alternative Steps} & \begin{tabitem}
			\item[2a.] If the submitted information is invalid or false, the admin discard the review (Test \ref{test:testDelete_Found:Draft}).
...
\begin{usecasetable}{8}{Validate Report}
	\begin{tabularx}{\textwidth}{@{} l|X @{}}
		...
		\textbf{Alternative Steps} & \begin{tabitem}
			...
			\item[4a.] \textbf{Extension (Ban User)}: If the content of the review meets the Ban Criteria, the admin gets the student ID related to the review and suspend the student's account.
		\end{tabitem}
	\end{tabularx}
\end{usecasetable}
